...
def test_soft_delta_task_preserves_baseline_and_never_reverses_task_order(
    score: float,
    ...
    debate = _make_debate(
        reward_a=1.0,
        reward_b=0.0,
        judge_raw_response=_soft_judge_audit(score),
    )
    split = assemble_split_train_examples(
        debates=[debate],
        num_rounds=1,
        round_adapter_names=("solution",),
        r1_reward_mode="judge_soft_delta_task",
        r23_reward_mode="none",
        r23_constant=1.0,
        r23_symmetric=True,
        task_reward_fn=lambda traj, _debate: float(traj.metrics["task_reward"]),
    ...
    rewards = {
        example.metadata["agent"]: example.metadata["r1_soft_reward"]
        for example in split["solution"]
    }
    assert rewards == {agent: pytest.approx(value) for agent, value in expected.items()}
    assert rewards["A"] >= rewards["B"]
    assert sum(rewards.values()) == pytest.approx(1.0)
    assert all(example.metadata["r1_task_order_cannot_reverse"] for example in split["solution"])
